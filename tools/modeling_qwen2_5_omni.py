...
_GLOBAL_SOURCE_H = None
_GLOBAL_SOURCE_W = None
_GLOBAL_FRAME_KEEP_INDICES = None
...
class Qwen2_5OmniPreTrainedModelForConditionalGeneration(Qwen2_5OmniPreTrainedModel):
    ...
    def get_llm_pos_ids_for_pruned_video(
        self,
        start_idx: int,
        num_frames: int,
        spatial_merge_size: int,
        ...
        source_h: int,
        source_w: int,
        frame_keep_indices: list,
    ):
        """
        Generate 3D position ids for pruned video frames.
        All frames use keep_indices to recover (row, col) in the original grid.
        """
        llm_pos_ids_list = []
        device = frame_keep_indices[0]['indices'].device if isinstance(frame_keep_indices[0]['indices'], torch.Tensor) else torch.device('cpu')
        
        for frame_idx in range(num_frames):
            info = frame_keep_indices[frame_idx]
            indices = info['indices']
            orig_w = info['llm_w_orig']
            orig_h = info['llm_h_orig']
            num_tokens_this_frame = len(indices)
            
            t_pos = int(frame_idx * second_per_grid * position_id_per_seconds)
            
            row = indices // orig_w
            col = indices % orig_w
            
            scale_ratio_h = source_h / (orig_h * spatial_merge_size)
            scale_ratio_w = source_w / (orig_w * spatial_merge_size)
            
            h_index = (scale_ratio_h * row.float()).round().long().to(device)
            w_index = (scale_ratio_w * col.float()).round().long().to(device)
            t_index = torch.full((num_tokens_this_frame,), t_pos, dtype=torch.long, device=device)
            
            frame_pos_ids = torch.stack([t_index, h_index, w_index]) + start_idx
            llm_pos_ids_list.append(frame_pos_ids)
        
        llm_pos_ids = torch.cat(llm_pos_ids_list, dim=1)
        return llm_pos_ids

    def get_pruned_video_token_count(
        self,
        num_frames: int,
        frame_keep_indices: list,
    ) -> int:
        """
        Count total tokens in a pruned video from frame_keep_indices.
        """
        total_tokens = 0
        for frame_idx in range(num_frames):
            total_tokens += len(frame_keep_indices[frame_idx]['indices'])
        return total_tokens


    def get_rope_index_pruning(
        self,
        ...
        audio_token_indices: Optional[np.ndarray] = None, 
        frame_keep_indices: list = None,  
            ...
            for i, input_ids in enumerate(total_input_ids):
                ...
                for _ in range(multimodal_nums):
                    ...
                    if min_ed == ed_audio:
                        ...
                        st_idx = llm_pos_ids_list[-1].max() + 1 if len(llm_pos_ids_list) > 0 else 0
                        bos_len = 1
                        llm_pos_ids_list.append(torch.arange(bos_len, device=device).view(1, -1).expand(3, -1) + st_idx)

                        st_idx = llm_pos_ids_list[-1].max() + 1 if len(llm_pos_ids_list) > 0 else 0
                        audio_len = ((audio_seqlens[audio_idx] - 1) // 2 + 1 - 2) // 2 + 1
                        llm_pos_ids = torch.arange(audio_len, device=device).view(1, -1).expand(3, -1) + st_idx
                        llm_pos_ids_list.append(llm_pos_ids)

                        st_idx = llm_pos_ids_list[-1].max() + 1 if len(llm_pos_ids_list) > 0 else 0
                        eos_len = 1
                        llm_pos_ids_list.append(torch.arange(eos_len, device=device).view(1, -1).expand(3, -1) + st_idx)
                        ...
                        t_index = (torch.arange(grid_t, device=device) * 1 * position_id_per_seconds).long()
                        ...
                        num_frames = len(frame_keep_indices)
                        llm_pos_ids = self.get_llm_pos_ids_for_pruned_video(
                            st_idx,
                            num_frames,
                            spatial_merge_size,
                            second_per_grids[video_idx].cpu().float().item(),
                            position_id_per_seconds,
                            source_h,
                            source_w,
                            frame_keep_indices=frame_keep_indices,
                        )
                        video_len = self.get_pruned_video_token_count(
                            num_frames,
                            frame_keep_indices=frame_keep_indices,
                        )
                        llm_pos_ids_list.append(llm_pos_ids)
                        ...
                        if audio_token_indices is not None:
                            indices = torch.from_numpy(audio_token_indices).to(device).long()
                            audio_len = len(indices)
                            audio_llm_pos_ids = indices.view(1, -1).expand(3, -1) + st_idx
                        else:
                            audio_len = ((audio_seqlens[audio_idx] - 1) // 2 + 1 - 2) // 2 + 1
                            audio_llm_pos_ids = torch.arange(audio_len, device=device).view(1, -1).expand(3, -1) + st_idx
                        

                        num_frames = len(frame_keep_indices)
                        
                        video_llm_pos_ids = self.get_llm_pos_ids_for_pruned_video(
                            st_idx,
                            num_frames,
                            spatial_merge_size,
                            second_per_grids[video_idx].cpu().float().item(),
...
class Qwen2_5OmniVisionEncoder(Qwen2_5OmniPreTrainedModel):
    ...
    def token_pruning(self, hidden_states, grid_thw, kwargs):
        ...
        frame_keep_indices = None 
        importance_scores = kwargs.get('score_list', None)
        target_prune_ratio = kwargs.get('target_prune_ratio', None)
        if importance_scores is not None:
            ...
            total_target = target_prune_ratio * T_total * llm_h_orig * llm_w_orig
            ...
            for i in range(num_groups):
                ...
                group_tokens = hidden_states[start:end]  # [n_tokens_per_group_orig, D]

                cur_token = min(target_tokens_per_group[i].item(), n_tokens_per_group_orig)
                aspect_ratio = float(llm_w_orig) / float(llm_h_orig)
                target_height = round((cur_token / aspect_ratio)**0.5)
                target_width = round(target_height * aspect_ratio)
                target_height = max(2, min(target_height, llm_h_orig))
                target_width = max(2, min(target_width, llm_w_orig))
                
                actual_frame_tokens = target_height * target_width

                if i % 2 == 1 and prev_group_tokens is not None:
                    num_keep = actual_frame_tokens
                    keep_ratio = actual_frame_tokens / n_tokens_per_group_orig
                    RATIO_THRESHOLD = 0.4  
                    if keep_ratio > RATIO_THRESHOLD:
                        with torch.no_grad():
                            normed = F.normalize(group_tokens, dim=1)
                            normed_prev = F.normalize(prev_group_tokens, dim=1)
                            
                            intra_sim = normed @ normed.T  # [N, N]
                            intra_sim.fill_diagonal_(-float('inf'))
                            intra_replaceability = intra_sim.max(dim=1).values


                            cross_sim = normed @ normed_prev.T  # [N_cur, N_prev]
                            inter_replaceability = cross_sim.max(dim=1).values
                            
                            total_replaceability = intra_replaceability * inter_replaceability
                            score = -total_replaceability
                        
                        if num_keep >= group_tokens.shape[0]:
                            keep_indices = torch.arange(group_tokens.shape[0], device=group_tokens.device)
                        else:
                            keep_indices = score.topk(num_keep).indices
                    
                    else:
                        cos_sim = F.cosine_similarity(group_tokens, prev_group_tokens, dim=1)
                        num_keep = actual_frame_tokens
                        keep_indices = cos_sim.topk(num_keep, largest=False).indices
                    
                    keep_indices = keep_indices.sort().values
                    resampled = group_tokens[keep_indices]
                    frame_keep_indices.append({
                        'indices': keep_indices,
                        'llm_w_orig': llm_w_orig,
                        'llm_h_orig': llm_h_orig,
                    })
                    ...
                    if num_keep >= group_tokens.shape[0]:
                        ...
                        with torch.no_grad():
                            ...
                            knn_vals, _ = torch.topk(sim, min(5, group_tokens.shape[0] - 1), dim=1)
                            density = knn_vals.mean(dim=1)
                            keep_indices = torch.topk(-density, num_keep).indices
                    keep_indices = keep_indices.sort().values
                    resampled = group_tokens[keep_indices]
                    frame_keep_indices.append({
                        'indices': keep_indices,
    ...
    def forward(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor, **kwargs) -> Tuple[torch.Tensor, Optional[torch.Tensor]]:
        ...
        hidden_states, frame_keep_indices = self.token_pruning(hidden_states, grid_thw, kwargs)
...
class Qwen2_5OmniThinkerForConditionalGeneration(Qwen2_5OmniPreTrainedModelForConditionalGeneration, GenerationMixin):
    ...
    def get_audio_features(
        ...
        if audio_compress_mask is not None:
            audio_features = self.compress_audio_features(audio_features, audio_compress_mask)
    ...
    def compress_audio_features(self, audio_features, compress_info, tokens_per_second=25):
        if compress_info is None or len(compress_info) == 0:
            return audio_features
        
        num_seconds = min(len(compress_info), audio_features.shape[0] // tokens_per_second)
        ...
        for sec_idx in range(num_seconds):
            sec_start = sec_idx * tokens_per_second
            sec_end = sec_start + tokens_per_second
            sec_features = audio_features[sec_start:sec_end]
            ...
            if target_tokens >= tokens_per_second:
                compressed_features.append(sec_features)
            else:
                r = tokens_per_second - target_tokens  
                merged = self._bipartite_soft_matching(sec_features, r)
                compressed_features.append(merged)
        
        processed_tokens = num_seconds * tokens_per_second
        if processed_tokens < audio_features.shape[0]:
            remaining = audio_features[processed_tokens:]
            compressed_features.append(remaining)
        
        return torch.cat(compressed_features, dim=0)


    

    def _bipartite_soft_matching(self, features, r):
        ...
        while current_features.shape[0] > target_N and current_features.shape[0] >= 2:
            current_N = current_features.shape[0]

            max_r_per_round = current_N // 2 -1
            r_this_round = min(current_N - target_N, max_r_per_round)
            
            if r_this_round <= 0:
                break
                
            current_features = self._bipartite_soft_matching_one_round(current_features, r_this_round)
        
        return current_features

    def _bipartite_soft_matching_one_round(self, features, r):
        ...
        src_idx = edge_idx[:r]           
        unm_idx = edge_idx[r:].sort()[0]
        dst_idx = node_idx[src_idx]    
        
        unmerged_a = a_raw[unm_idx]
        src = a_raw[src_idx]
        merged_b = b_raw.clone()
        b_counts = torch.ones(len_b, 1, device=device, dtype=dtype)

        dst_cpu = dst_idx.cpu().tolist()
        groups = {}
        for i, d in enumerate(dst_cpu):
            groups.setdefault(d, []).append(i)

        for d, src_indices in groups.items():
            idx_t = torch.tensor(src_indices, device=device)
            merged_b[d] += src[idx_t].sum(dim=0)
            b_counts[d] += len(src_indices)

        merged_b = merged_b / b_counts
        
        idx_a = torch.arange(0, N, 2, device=device)
        idx_b = torch.arange(1, N, 2, device=device)
        

        idx_unm = idx_a[unm_idx]

        idx_merged = idx_b
        

        output_indices = torch.cat([idx_unm, idx_merged], dim=0)
        perm = output_indices.argsort()
        
        output_features = torch.cat([unmerged_a, merged_b], dim=0)
        return output_features[perm]
    ...
    def forward(
        ...
        target_prune_ratio: Optional[int] = None,
        cached_video_embeds: Optional[torch.Tensor] = None,
        cached_video_grid_thw: Optional[torch.LongTensor] = None,

        audio_compress_mask=None,
        audio_token_indices=None,
        cached_audio_embeds: Optional[torch.Tensor] = None,
        ...
        if input_ids is not None and input_ids.shape[1] != 1:  # Prefill stage
            if cached_audio_embeds is not None:
                audio_features = cached_audio_embeds
                if audio_compress_mask is not None:
                    audio_features = self.compress_audio_features(audio_features, audio_compress_mask)
            ...
            if pixel_values_videos is not None or cached_video_embeds is not None:

                global _GLOBAL_SOURCE_H
                global _GLOBAL_SOURCE_W
                global _GLOBAL_FRAME_KEEP_INDICES
                if cached_video_embeds is not None and pixel_values_videos is None:

                    source_h = cached_video_grid_thw[0][1].item()
                    source_w = cached_video_grid_thw[0][2].item()
                    
                    video_embeds, frame_keep_indices = self.visual.token_pruning(
                        cached_video_embeds, cached_video_grid_thw,
                        {'score_list': score_list, 'target_prune_ratio': target_prune_ratio}
                    )

                    
                else:
                    pixel_values_videos = pixel_values_videos.type(self.visual.dtype)
                    source_h = video_grid_thw[0][1].item()
                    source_w = video_grid_thw[0][2].item()
                    video_embeds, frame_keep_indices = self.visual(pixel_values_videos, grid_thw=video_grid_thw, score_list=score_list, target_prune_ratio=target_prune_ratio)
                

                _GLOBAL_SOURCE_H = source_h
                _GLOBAL_SOURCE_W = source_w
                _GLOBAL_FRAME_KEEP_INDICES = frame_keep_indices
        ...
        if attention_mask is not None and position_ids is None:
            if (
                ...
                if frame_keep_indices is not None:

                    position_ids, rope_deltas = self.get_rope_index_pruning(
                        input_ids,
                        image_grid_thw,
                        video_grid_thw,
                        attention_mask,
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        # model_inputs["cached_video_embeds"] = cached_video_embeds
        # model_inputs["cached_video_grid_thw"] = cached_video_grid_thw
        # model_inputs["score_list"] = score_list
        # model_inputs["target_prune_ratio"] = target_prune_ratio

        # if cache_position[0] != 0:
        #     model_inputs["pixel_values"] = None
        #     model_inputs["pixel_values_videos"] = None
        #     model_inputs["cached_video_embeds"] = None
        #     model_inputs["cached_video_grid_thw"] = None

        if cache_position[0] != 0:
            model_inputs["pixel_values"] = None
            model_inputs["pixel_values_videos"] = None
            model_inputs["cached_audio_embeds"] = None
        ...
        if attention_mask is not None and position_ids is None:
            if (
                ...
                position_ids, rope_deltas = self.get_rope_index_pruning(
                    ...
                    _GLOBAL_SOURCE_H,
                    _GLOBAL_SOURCE_W,
                    frame_keep_indices=_GLOBAL_FRAME_KEEP_INDICES,
                )                
    ...
    def generate(
        ...
        **kwargs,
        ...
        Args:
            ...
            cached_audio_embeds (`Optional[torch.Tensor]`, *optional*):
                Pre-computed audio embeddings from audio_tower. If provided, skips audio_tower encoding.
            kwargs (*optional*):
