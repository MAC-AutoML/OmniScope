...
class Qwen2_5OmniVisionEncoder(Qwen2_5_VisionTransformerPretrainedModel):
    ...
    # sjs :
    def token_pruning(self, hidden_states, grid_thw, kwargs):
        new_grid_thw = None
        importance_scores = kwargs.get('score_list', None)
        target_prune_frames = kwargs.get('target_prune_frames', None)
        if target_prune_frames is not None:
            total_target = int(hidden_states.shape[0] * target_prune_frames / grid_thw[0][0])
        if importance_scores is not None:
            s = self.spatial_merge_size
            T_total, H_patch, W_patch = grid_thw[0].tolist()
            num_groups = T_total

            # merger 后每组原始 token 数: (H_patch // s) * (W_patch // s)
            llm_h_orig, llm_w_orig = H_patch // s, W_patch // s
            n_tokens_per_group_orig = llm_h_orig * llm_w_orig

            # 每组边界
            group_boundaries = [(i * n_tokens_per_group_orig, (i + 1) * n_tokens_per_group_orig) for i in range(num_groups)]
            # 分配基础数量
            target_tokens_per_group = (importance_scores / importance_scores.sum() * total_target)
            target_tokens_per_group = target_tokens_per_group.round().int()
            resampled_tokens = []
            new_grid_thw = []
            num_tokens = 0
            for i in range(num_groups):
                start, end = group_boundaries[i]
                group_tokens = hidden_states[start:end]
                cur_token = target_tokens_per_group[i].item()

                orig_h, orig_w = llm_h_orig, llm_w_orig
                aspect_ratio = orig_w / orig_h

                target_height = int((cur_token / aspect_ratio)**0.5)
                ...
                H = s * target_height
                W = s * target_width
                num_tokens += target_height * target_width

                if cur_token == group_tokens.shape[0] and group_tokens.shape[0] == target_height * target_width:
                    resampled = group_tokens
                else:
                    x = group_tokens.view(llm_h_orig, llm_w_orig, -1)
                    x = x.permute(2, 0, 1).unsqueeze(0)
                    # resampled = F.interpolate(x, size=scaled_shape, mode='bilinear')
                    resampled = F.interpolate(x, size=scaled_shape, mode='bicubic', align_corners=False)
                    resampled = resampled.squeeze(0).permute(1, 2, 0)
                    resampled = resampled.reshape(-1, resampled.size(-1))

                resampled_tokens.append(resampled)
                new_grid_thw.append([1, H, W])

            hidden_states = torch.cat(resampled_tokens, dim=0)
            new_grid_thw = torch.tensor([new_grid_thw], dtype=grid_thw.dtype, device=grid_thw.device)

        return hidden_states, new_grid_thw

    # def forward(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor) -> torch.Tensor:
    def forward(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor, **kwargs) -> Tuple[torch.Tensor, Optional[torch.Tensor]]:
        """
        Args:
        ...
        # sjs : 动态压缩视觉 token 并重构 grid_thw
        hidden_states, new_grid_thw = self.token_pruning(hidden_states, grid_thw, kwargs)
...
class Qwen2_5OmniThinkerForConditionalGeneration(Qwen2_5OmniPreTrainedModelForConditionalGeneration, GenerationMixin):
    ...
    def forward(
        ...
        score_list: Optional[torch.Tensor] = None,
        target_prune_frames: Optional[int] = None,
        ...
        if input_ids is not None and input_ids.shape[1] != 1:  # Prefill stage
            ...
            if pixel_values_videos is not None:
                ...
                # sjs : 1
                video_embeds, new_grid_thw = self.visual(pixel_values_videos, grid_thw=video_grid_thw, score_list=score_list, target_prune_frames=target_prune_frames)
                if new_grid_thw is not None:
                    source_h = video_grid_thw[0][1]
